...
function Poster({ game }: { game: CasinoGame }) {
  ...
  return (
    <div className={`group relative aspect-[0.78] overflow-hidden rounded-[1.45rem] bg-gradient-to-br ${art.gradient} ring-1 ring-white/20 shadow-[0_18px_40px_rgba(18,14,52,0.7)] transition duration-200 hover:-translate-y-0.5`}>
      ...
      {game.isHot && (
        <span className="absolute left-2 top-10 z-10 inline-flex items-center gap-1 rounded-full bg-orange-500 px-1.5 py-1 text-[9px] font-black uppercase text-white shadow">
          <Flame className="h-3 w-3" />
          Hot
        </span>
      )}

      <div className="absolute inset-x-0 top-[19%] flex justify-center">
      ...
      <div className="absolute inset-x-0 bottom-0 z-10 px-2 pb-2 pt-10 text-center">
...
export const CasinoView: React.FC = () => {
  ...
  const games = useMemo(
    () =>
      CASINO_GAMES.filter(
        game => (category === 'all' || game.category === category) && game.name.toLowerCase().includes(query.toLowerCase())
      ),
    [category, query]
  );
  ...
  return (
    <section className="min-h-[calc(100vh-7rem)] space-y-4 rounded-[2rem] bg-[#0b0e18] p-3 text-white sm:p-5">
      <div className="relative overflow-hidden rounded-[1.8rem] border border-yellow-300/40 bg-gradient-to-br from-[#35126e] via-[#20143d] to-[#101a3d] p-4 shadow-[0_18px_55px_rgba(101,49,190,.25)]">
        ...
        <div className="relative flex items-center justify-between">
          <div>
            <div className="text-[10px] font-black uppercase tracking-[.28em] text-yellow-300">FortuneGo</div>
            <div className="mt-1 text-xl font-black tracking-tight sm:text-2xl">Casino <span className="text-pink-300">Live</span></div>
          </div>
        ...
        <div className="relative mt-3 grid grid-cols-3 gap-2 text-center text-white">
          {[
            ['MAJOR', '975.030,79', 'from-red-600 to-rose-700'],
            ['MINOR', '2,00', 'from-blue-600 to-indigo-700'],
            ['MINI', '1,00', 'from-emerald-600 to-green-700'],
          ].map(([label, value, color]) => (
            <div key={label} className={`rounded-xl border border-white/10 bg-gradient-to-r ${color} px-2 py-2`}>
              <div className="text-[9px] font-black uppercase tracking-[.18em] text-white/90">{label}</div>
          ...
          <button
            ...
            className={`flex shrink-0 items-center gap-1.5 rounded-xl px-3 py-2 text-[11px] font-black transition ${
              category === item.id
                ...
                : 'bg-[#1b1d27] text-slate-300 hover:bg-[#242836]'
            }`}
          >
            {item.icon}
            {item.label}
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full rounded-2xl border border-white/10 bg-[#151923] py-3 pl-9 pr-4 text-sm text-white placeholder:text-slate-500 outline-none ring-0"
        />
      </div>
      ...
      {games.length === 0 && (
        <div className="rounded-2xl border border-dashed border-white/10 py-12 text-center text-sm text-slate-500">Nenhum jogo encontrado.</div>
      )}

      <div className="flex items-center justify-center gap-2 border-t border-white/10 pt-3 text-[10px] text-slate-500">
        <ShieldCheck className="h-3.5 w-3.5 text-emerald-400" />
        Ambiente demonstrativo • Jogos com saldo real em sandbox
      </div>
      ...
      >
        <span className="flex items-center gap-2">
          ...
          <span>
            <span className="block text-[10px] uppercase tracking-[.2em] text-slate-500">Carteira</span>
            <span className="text-sm font-black text-white">Saldo: R$ {user.balance.toFixed(2)}</span>
          </span>
        </span>
